...
- Working estimate (risk-weighted): ~67% complete (range 63-71%).
...
- #136: ISA+parser tranche 29/30 (known-head no-cascade safeguard + expanded ED/CB/zero-operand hardening + `(ix+disp)/(iy+disp)` parity + malformed control-keyword parser recovery matrix).
